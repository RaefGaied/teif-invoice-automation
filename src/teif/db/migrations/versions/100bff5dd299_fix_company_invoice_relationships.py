"""fix company-invoice relationships

Revision ID: 100bff5dd299
Revises: 1ee1ab3c5618
Create Date: 2025-09-16 13:59:22.942106
...
def upgrade():
    ...
    op.alter_column('invoice_dates', 'created_at',
               ...
               type_=sa.DateTime(),
               nullable=False)
    op.add_column('invoice_lines', sa.Column('discount_percent', sa.Numeric(precision=5, scale=2), nullable=True, comment='Discount percentage (0-100)'))
    op.add_column('invoice_lines', sa.Column('additional_info', sa.JSON(), nullable=True, comment='Additional line-specific information in JSON format'))
    ...
    op.add_column('invoices', sa.Column('document_status', sa.String(length=20), nullable=False, comment='Current status of the invoice'))
    op.add_column('invoices', sa.Column('delivery_party_id', sa.Integer(), nullable=True, comment='Delivery party ID if different from customer (I-66)'))
    op.add_column('invoices', sa.Column('payment_terms', sa.Text(), nullable=True, comment='Payment terms and conditions (I-37)'))
    op.add_column('invoices', sa.Column('payment_means_code', sa.String(length=10), nullable=True, comment='Payment means code (I-38)'))
    op.add_column('invoices', sa.Column('payment_means_text', sa.String(length=255), nullable=True, comment='Payment means description'))
    op.add_column('invoices', sa.Column('additional_info', sa.JSON(), nullable=True, comment='Additional TEIF-specific information in JSON format'))
    ...
    op.create_foreign_key(None, 'invoices', 'companies', ['delivery_party_id'], ['id'])
    op.create_table_comment(
        'invoices',
...
def downgrade():
    ...
    op.add_column('invoices', sa.Column('status', sa.NVARCHAR(length=50, collation='SQL_Latin1_General_CP1_CI_AS'), server_default=sa.text("('processing')"), autoincrement=False, nullable=False))
    op.drop_table_comment(
        'invoices',
    ...
    op.drop_column('invoices', 'additional_info')
    op.drop_column('invoices', 'payment_means_text')
    op.drop_column('invoices', 'payment_means_code')
    op.drop_column('invoices', 'payment_terms')
    op.drop_column('invoices', 'delivery_party_id')
